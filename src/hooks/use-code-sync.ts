...
export function useCodeSync({
    ...
    useEffect(() => {
        if (!monacoEditor || !elements.length || !selectedElementIds.length || lastSyncSource !== 'visual') {
            return;
        }
    ...
    }, [selectedElementIds, monacoEditor, elements, lastSyncSource]);
            ...
            const matchingElements = elements.filter(el => {
                ...
                return (
                    (selection.startLineNumber < elEnd.lineNumber ||
                        (selection.startLineNumber === elEnd.lineNumber && selection.startColumn <= elEnd.column)) &&
                    (selection.endLineNumber > elStart.lineNumber ||
                        (selection.endLineNumber === elStart.lineNumber && selection.endColumn >= elStart.column))
                );
            });
    ...
    }, [monacoEditor, elements, isSyncing, lastSyncSource, onVisualElementHighlight]);
